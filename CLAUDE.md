...
- **Chini citation** — 🔲 open (Jesse). The NAD+/CD38 reference: Dr. Eduardo Chini (Mayo Clinic), CD38 as a driver of age-related NAD+ decline — the science backing Apigenin (CD38 inhibitor) + NMN (NAD+ precursor) in EQ:Focus. Appears as a DOI link in `evidence/index.html` (`10.1038/s41574-020-0355-2`, *Nature Reviews Endocrinology* 2020) and inline + full reference in the Learn NAD+ article. **Possible mismatch to reconcile:** the Learn full reference quotes a title ("CD38 ecto-enzyme in immune cells is induced during aging…") that may belong to a different (earlier Cell Metabolism) Chini paper than the 2020 NRE review the DOI points to. Verify against the source.
